...
Before writing code, I reduced the assignment to two transformations:

1. Convert the recursive tree received by the API into a form PostgreSQL can
   compare and update efficiently.
2. Convert the stored database rows back into the requested recursive subtree.

This separation keeps the API natural for clients while giving the database a
relational representation that is easier to constrain, query, and update.

### Represent the tree as parent-child rows

Consider this hierarchy:

```mermaid
flowchart TD
    Root["1 · management_group<br/>root"]
    SubscriptionA["2 · subscription<br/>parent: 1 · position: 0"]
    ResourceGroup["3 · resource_group<br/>parent: 2 · position: 0"]
    SubscriptionB["4 · subscription<br/>parent: 1 · position: 1"]

    Root --> SubscriptionA
    SubscriptionA --> ResourceGroup
    Root --> SubscriptionB
```

The same hierarchy can be drawn as:
...
Instead of storing that entire recursive object in one database cell, store one
row per node:

| `id` | `root_id` | `parent_id` | `node_type` | `sibling_position` |
| ---: | ---: | ---: | --- | ---: |
| 1 | 1 | `NULL` | `management_group` | 0 |
| 2 | 1 | 1 | `subscription` | 0 |
| 3 | 1 | 2 | `resource_group` | 0 |
| 4 | 1 | 1 | `subscription` | 1 |

The relationships can now be read directly from the rows:

- Node `1` has no parent, so it is the root.
- Node `2` has `parent_id = 1`, so it is a child of node `1`.
- Node `3` has `parent_id = 2`, so it is a child of node `2`.
- Node `4` also has `parent_id = 1`, so it is another child of node `1`.
- There is no `children` column. The children of node `1` are found by looking
  for rows whose `parent_id` equals `1`.
- Nodes `2` and `4` share a parent. Their positions preserve their original
  left-to-right order.
- The root uses position `0` as a consistent stored value; position matters
  when ordering nodes that share a parent.
- Every row has `root_id = 1`, which identifies the complete hierarchy that
  owns it.

The important idea is simply that every child remembers the ID of its direct
parent. This relational tree representation is commonly called an
**adjacency list**.

### Treat POST as the latest complete snapshot

The requirement says an update may add, remove, or move nodes and that stored
data must always represent the latest update. I therefore treat each
`POST /hierarchy` document as the complete desired state for its root, not as a
partial patch.

The planned write algorithm was:

1. Validate the complete input tree and flatten it into parent-child rows.
2. Begin one database transaction and lock the hierarchy root.
3. Bulk-copy the incoming rows into a temporary staging table.
4. Compare the incoming set with the stored set:
   - insert IDs that are new;
   - update rows whose parent, type, or position changed;
   - delete stored IDs missing from the new snapshot.
5. Commit only after every step and database constraint succeeds.

The temporary table is the bridge between recursive application input and
set-based SQL. It avoids issuing one independent database request per node and
lets PostgreSQL reconcile the complete snapshot as a set.

### Retrieve rows first, then rebuild the tree

For `GET /hierarchy/{node_id}`, PostgreSQL starts with the requested node and
uses a recursive query to find its children, their children, and so on. The
query returns flat rows. Go then groups those rows by `parent_id`, sorts each
group by `sibling_position`, and rebuilds the recursive `children` arrays.

```mermaid
flowchart LR
    Input["Recursive JSON tree"]
    Flatten["Validate and flatten"]
    Rows["Parent-child rows"]
    Reconcile["Transactional reconciliation"]
    Database[("PostgreSQL")]
    Query["Recursive subtree query"]
    Build["Group by parent and rebuild"]
    Output["Recursive JSON subtree"]

    Input --> Flatten --> Rows --> Reconcile --> Database
    Database --> Query --> Build --> Output
```

This provides a tree-shaped API without requiring the database to store or
manipulate one large recursive JSON document.

### Check common changes against the model

Before implementation, I checked whether the representation handled every
required update naturally:

| Change | Relational effect |
| --- | --- |
| Add a node | Insert one row with its parent ID and sibling position |
| Remove a node | Delete rows absent from the latest snapshot |
| Move a node | Change that node's `parent_id`; descendants do not need rewriting |
| Rename or retype a node | Change its `node_type` |
| Reorder children | Change their `sibling_position` values |
| Submit an identical snapshot | Keep identical node rows unchanged |

### Define correctness and concurrency before optimizing

The approach also needed to remain correct when input is invalid or requests
overlap:

- Validate positive and unique IDs, non-empty types, explicit child arrays,
  maximum depth, and maximum node count before writing.
- Use database constraints to ensure one root, valid same-hierarchy parents,
  global node ownership, and unique sibling positions.
- Perform reconciliation in one transaction so failures never expose a partial
  hierarchy.
- Lock one root while replacing it so concurrent snapshots for that root cannot
  mix together.
- Reject a node ID already owned by another root.
- Bound request bodies, operation time, and database lock waits.

For a hierarchy containing `n` nodes, validation and flattening are `O(n)`.
The rows are transferred with `COPY`, and reconciliation uses a fixed sequence
of set-based SQL operations. A subtree containing `k` nodes is loaded with one
recursive query and reconstructed in memory. This avoids N+1 database queries
and unnecessary descendant rewrites when a node moves.

The remaining sections show how this approach is implemented and tested.
